...
contract BEP721 is Context, BEP165, IBEP721, IBEP721Metadata, Ownable {
    ...
    //Mapping token ID to data json hash
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, _hash[tokenId]))
                : "";
    }
     ...
     * @dev mints a new NFT.
     */
    function mint(uint256 tokenId, string memory hash) public virtual {
        _safeMint(_msgSender(), tokenId, hash);
    }
